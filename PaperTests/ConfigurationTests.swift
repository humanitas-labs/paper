...
struct ThemeTests {
    ...
    func themeKeysParseAndOverridesApply() {
        ...
        #expect(Configuration().menuBar, "the menu bar item is on by default")
        #expect(!Configuration.parse("menu.bar = off").menuBar)
        #expect(Configuration.parse("menu.bar = off").entries.contains { $0.key == "menu.bar" && $0.value == "off" })
        #expect(Configuration.template.contains("menu.bar = on"))
