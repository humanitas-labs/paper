...
struct ThemeTests {
    ...
    func themeKeysParseAndOverridesApply() {
        ...
        #expect(!Configuration().menuBar, "the menu bar item is off by default")
        #expect(Configuration.parse("menu.bar = on").menuBar)
        #expect(Configuration.parse("menu.bar = on").entries.contains { $0.key == "menu.bar" && $0.value == "on" })
        #expect(Configuration.template.contains("menu.bar = off"))
